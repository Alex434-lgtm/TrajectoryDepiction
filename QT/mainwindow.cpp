...
{
    // Stop CCCP communication if running
...
{
    // Skip updates if using live telemetry mode
    if (m_useLiveTelemetry) {
        return;  // Position updates handled by CCCP callbacks
    }
...
void MainWindow::RocketOpenGLWidget::paintGL()
{
    ...
    // Update marker color transition point based on acceleration
    if (a[currentIndex] < 0) {
        // Acceleration is negative (coast phase)
    } else {
        staticplaceholder++;
    }
...
{
    // Simple world-space camera translation
    cameraX += dx;
    cameraY += dy;
    cameraZ += dz;
    update();
...
//---------------------------------------
// Setup CCCP Communication
//---------------------------------------
void MainWindow::setupCCCP() {
    // Create CCCP communication manager
    m_cccp = std::make_unique<CCCPManager>(this);

    // Connect telemetry data signals
    ...
    // Create telemetry data display label
    ...
    // Initialize CCCP but don't start automatically
...
void MainWindow::onAttitudeData(const AttitudeData& data) {
    ...
    if (m_useLiveTelemetry && glWidget) {
        // Convert attitude data to quaternion
